...
class NavTop extends React.Component{
    constructor(props,context){
        ...
        // //在nav组件的构造函数中，从服务器获取购物车信息，并存储到redux中
        this.props.queryUnpay();
        this.props.queryPay();

    }
    //class类里面的函数一定要用箭头函数，因为只有用箭头函数，函数里面的this才指向实例，不然会报错没有props属性
    handleClick=(ev)=>{
    ...
    {/* transition是react-transition-group插件中的组件，是用来实现过渡动画的，其属性in是用来控制显示和隐藏的，当in的值改变，transition组件会重新渲染(属性改变组件渲染)，
    会把transition里面的函数执行，如果in为true,...transitionStyles[state]=...transitionStyles[0]拿到的是transitionStyles的第一个数，也就是opacity:0，
    再把这个赋给ul的style，所以ul的opacity为0，则隐藏。若in为true，则...transitionStyles[state]=...transitionStyles[1]，拿到的是transitionStyles的第一个数，
    也就是opacity:1，再把这个赋给ul的style，所以ul的opacity为1，则显示所以react-transtion-group也是通过控制透明度来控制显示和隐藏，并且是通过转态来控制的*/}
    {/* 在transition里将in设置为this.state.in（也就是该组件构造函数里定义的状态中的in，设置默认值为false（隐藏）,即this.state={in:false}），
          以后点击的时候把this.state.in从false和true来回切换即可实现显示和隐藏, */}
