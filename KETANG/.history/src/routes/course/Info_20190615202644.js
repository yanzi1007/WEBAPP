...
class Info extends React.Component{
    ...
    async componentDidMount(){
        ...
        //发送请求获取要在详情页展示的信息 result中的data就是
        let result=await queryInfo(courseId); //课程详情页展示的信息是直接向服务器发送请求得到，没有存到redux中
        let {code} = result;
        if(parseFloat(code)===0){
            /*校验当期课程是已支付还是未支付或者是未加入购物车，用isShop表示*/
            ...
            this.setState({
                data: result.data, //更改课程详情页的信息
                isShop //更改是否已经加入购物车的信息
            });
        }
    }
    ...
    //写成箭头函数，保证这个函数里面的this是当前类的实例
    handleShopCart=async ev=>{
        if(this.state.isShop===-1){
            //还未加入购物车(按钮是加入购物车)
            let result=await addShopCart(this.courseId); //发送请求
            if(parseFloat(result.code)===0){
                this.props.queryUnpay();//通过dispatch任务派发通知容器中的购物车信息进行更新
                //页面重新展示最新样式
                this.setState({
                    isShop:0
                })
        ...
        //已经加入购物车（按钮是移除购物车）
        let result=await removeShopCart(this.courseId); //发送请求
            if(parseFloat(result.code)===0){
                this.props.queryUnpay(); //通过dispatch任务派发通知容器中的购物车信息进行更新
                //页面重新展示最新样式
                this.setState({
