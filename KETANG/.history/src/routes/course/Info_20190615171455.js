...
class Info extends React.Component{
    ...
    async componentDidMount(){
        ...
        let {unpay,pay}=this.props.shopCart,isShop=-1;
            /*在redux容器中的未购买和已购买的集合中查找是否有当前展示的课程，有的话说明已经加入到购物车，没有的话说明未加入*/
            let r1=unpay.find(item=>{return parseFloat(item.id)==parseFloat(courseId)})
            
            r1!==undefined? isShop=0:null;
            let r2=pay.find(item=>{return parseFloat(item.id)==parseFloat(courseId)})
            r1!==undefined ? isShop=1:null;
